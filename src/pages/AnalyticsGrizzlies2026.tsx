  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="container space-y-8 pb-16 pt-28">
        <section className="flex flex-col gap-5 md:flex-row md:items-center"><img src="/grizzlies-2026-logo.png" alt="San Ramon Grizzlies" className="h-24 w-24 object-contain" /><div><h1 className="font-display text-4xl font-bold leading-tight md:text-5xl">Grizzlies 2026 Analytics</h1><p className="mt-1 font-display text-xl font-semibold md:text-2xl"><span className="text-foreground">{grizzliesPoweredBy.prefix}</span><span className="text-gradient-primary">{grizzliesPoweredBy.accent}</span></p></div><div className="font-display font-bold leading-tight text-white md:ml-auto md:text-right"><p className="text-xl md:text-2xl"><span>{welcome.greetingPrefix}</span>{welcome.greetingName ? <span className="text-gradient-primary">{welcome.greetingName}</span> : null}</p><p className="mt-1 text-xl md:text-2xl"><span>{welcome.seasonPrefix}</span><span className="text-red-500">{welcome.teamName}</span><span>{welcome.seasonSuffix}</span></p></div></section>
