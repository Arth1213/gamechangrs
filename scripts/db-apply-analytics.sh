...
PSQL_BIN="$(find_psql)"
resolve_target_config "analytics"
resolve_db_url "analytics"
load_manifest
...
VALUES ('analytics', '${version}', '${escaped_file}', '${escaped_checksum}', 'Baseline inserted by $(basename "$0")')
...
echo "Applying pending analytics migrations..."
for idx in "${!MANIFEST_FILES[@]}"; do
  version="${MANIFEST_VERSIONS[$idx]}"
  manifest_file="${MANIFEST_FILES[$idx]}"
  if in_array "$version" "${APPLIED_VERSIONS[@]}"; then
...
\i $abs_file
INSERT INTO ${TRACKING_TABLE} (target, version, filename, checksum_sha256, notes)
VALUES ('analytics', '${version}', '${escaped_file}', '${escaped_checksum}', 'Applied by $(basename "$0")')
ON CONFLICT (target, version) DO NOTHING;
COMMIT;
EOF
done
